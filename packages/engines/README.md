...
| `pull-request` (real; Capability Sprint 16) | `PullRequestEngine`/`buildPullRequestProposal()` — the "PROPOSE CHANGE" stage: converts `adaptive-decision`'s `EngineeringDecision` plus the run's already-computed implementation artifacts into one deterministic `PullRequestProposal` (title, summary, full markdown body, suggested `oram/<slug>` branch name, decision/risk/validation score, proposed changes, verification expectations, human-approval flag). NOT the GitHub publisher itself: never calls GitHub, never runs git, never invokes an LLM, never writes a file, never applies a patch — that is `publisher`'s job (below), and even it only simulates by default (Capability Sprint 20). `STOP` (or zero ImplementationRequests) produces an explicit `NO_ACTION` proposal. Not built on any legacy script. |
...
| `publisher` (real; Capability Sprint 20) | `PublisherEngine`/`buildPublishRecord()` — the "Publisher" half of the PUBLISHING phase (`pull-request` above is the other half): converts `pull-request`'s `PullRequestProposal` into one deterministic `PublishRecord` via a pluggable `PublisherClient` (four stages: Create Branch, Commit, Push, Create Draft Pull Request). `MemoryPublisher` (the default — deterministic, ALWAYS dry-run, no git/GitHub CLI/network) and `GitHubPublisher` (every method throws the SAME `NotImplementedYetError` reused, read-only, from `implementation-executor`'s `RealAdapter`; never the default). A `NO_ACTION` proposal, or one still requiring its own human approval, is `SKIPPED` — no stage attempted. Modeled directly on `scripts/github-publisher.js` + `publisher/github/client.js` (real, tested, dry-run-by-default — the same two files this table's own previous `(future)` row for this capability named as the intended starting point); `scripts/agent-branch-publish.js` was not used as a reference (superseded by the unified `github-publisher.js` path per the migration plan). |
...
**Capability Sprint 20 (current) — Publisher Engine:** added `publisher` -- the "Publisher" half of
Lifecycle.ts's own PUBLISHING phase mapping ("Pull Request Generator + Publisher"; Sprint 16 built the
first half). `PublisherEngine`/`buildPublishRecord()` converts `pull-request`'s `PullRequestProposal` into
one deterministic `PublishRecord` (outcome, reason, branch, the four publishing stages -- Create Branch,
Commit, Push, Create Draft Pull Request -- and a pull request URL), modeled directly on
`scripts/github-publisher.js` + `publisher/github/client.js` (System A's real, tested, dry-run-by-default
GitHub adapter -- named in `ROADMAP.md` as this package's own intended starting point). Ships with
`MemoryPublisher`, a deterministic, ALWAYS-dry-run `PublisherClient` (no git, no GitHub CLI, no filesystem
write, no network call -- the same "real, deterministic default" role `MemoryProvider`/`MemoryAdapter`
already play for their own stages) plus a `GitHubPublisher` stub (reuses `NotImplementedYetError`,
unconditionally throws, never the default -- real git/GitHub operations remain out of reach for a
structural reason too: `ImplementationRequest.implementationTargets.files` is always empty today, so there
is no real file-level diff yet for a real Commit stage to stage). A proposal that is `NO_ACTION` or still
has `humanApprovalRequired: true` is `SKIPPED` -- no stage attempted, mirroring
`assertApprovedForPR`'s own gate from the legacy script, done gracefully rather than by throwing. Machine-
independent by construction (reuses `memory`'s own `makeRepositoryId`). Follows Sprint 17's artifact-handoff
convention (`PUBLISHER_UPSTREAM_ARTIFACTS`, `loadPublisherInputsFromRun()`, a throwing recompute fallback
inside the full pipeline) and finally puts `@oram/events`' own `PRCreatedEvent` to use -- typed since Phase
1, unused until now. Wired as the full pipeline's 14th and final stage (`@oram/core`'s
`FULL_ENGINEERING_WORKFLOW`), inside the same post-approval segment Capability Sprint 19's real safety gate
already protects Provider Execution with. Also added the CLI's `oram publish <path>` command and extended
`oram run`/`oram engineer` with a PUBLISH RECORD section. See `publisher.test.ts` (11 tests: publish
outcomes per proposal, MemoryPublisher determinism, environment-leak guard, a stored JSON snapshot, a
real-repository smoke test), `publisher.runtime.test.ts` (2 tests: producer→consumer handoff, recompute
fallback), and `full-pipeline.runtime.test.ts`'s own extended coverage of the real end-to-end gate.

**Capability Sprint 18 — Full Runtime Pipeline:** every remaining engine wrapper's
...
`PullRequestProposal`: title, summary, full markdown PR body, suggested branch name
(`oram/<primary-request-slug>`), base branch, decision/risk/validation score, proposed changes, verification
expectations, and whether human approval is required (`ESCALATE_TO_HUMAN`, `STOP`, or HIGH risk). A `STOP`
decision (or a run with zero ImplementationRequests) produces an explicit `NO_ACTION` proposal -- no branch,
and a body stating that no implementation PR should be created. This is explicitly NOT the GitHub publisher:
the engine never calls GitHub, never runs git, never invokes an LLM, never writes a file, never applies a
patch -- that became `publisher`'s job in Capability Sprint 20, and even it only simulates by default.
Machine-independent by construction
(reuses `memory`'s own `makeRepositoryId`; no absolute path ever appears in a proposal -- pinned by its own
test). Also added the CLI's `oram pull-request <path>` command and extended the flagship `oram engineer`
report with the PULL REQUEST PROPOSAL section. See `pull-request.test.ts` (12 tests: proposal kinds per
decision, human-approval policy, branch/body determinism, environment-leak guard, a stored JSON snapshot, a
real-repository smoke test) plus the CLI's own `renderPullRequestReport.test.ts`.

**Capability Sprint 14:** added `adaptive-decision` -- the first stage in this pipeline that
