...
test("web-app fixture: full-stack Node/React/Express app is detected with rich, evidence-backed metadata", () => {
  ...
  assert.deepEqual(new Set(analysis.dependencySummary.manifests[0]?.dependencyNames), new Set(["react", "react-dom", "express", "mongoose", "jsonwebtoken", "bcrypt", "openai", "@aws-sdk/client-s3", "jest", "typescript", "webpack"]));
  for (const detection of [...analysis.frameworks, ...analysis.apiFrameworks, ...analysis.buildTools, ...analysis.testingFrameworks, ...analysis.databaseTechnologies, ...analysis.authenticationLibraries, ...analysis.aiLlmLibraries, ...analysis.cloudProviders, ...analysis.packageManagers, analysis.projectType, analysis.docker, analysis.monorepo]) {
...
test("minimal fixture: no evidence anywhere yields honest Unknown/empty results, never a guess", () => {
  ...
  for (const detections of [analysis.frameworks, analysis.apiFrameworks, analysis.buildTools, analysis.testingFrameworks, analysis.packageManagers, analysis.configurationFiles, analysis.environmentFiles, analysis.ciCdSystems, analysis.infrastructureFiles, analysis.databaseTechnologies, analysis.authenticationLibraries, analysis.aiLlmLibraries, analysis.cloudProviders, analysis.deploymentTargets, analysis.entryPoints]) {
...
test("identity is deterministic: analyzing the same fixture twice produces byte-identical ids", () => {
  ...
  assert.deepEqual(first.frameworks.map((d) => d.id).sort(), second.frameworks.map((d) => d.id).sort());
  assert.equal(first.projectType.id, second.projectType.id);
  assert.equal(first.docker.id, second.docker.id);
  assert.deepEqual(first.repositoryStructure.map((e) => e.id).sort(), second.repositoryStructure.map((e) => e.id).sort());
});
